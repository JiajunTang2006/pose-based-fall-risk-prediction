...
def main() -> None:
    ...
    parser.add_argument("--prefall-frames", type=int, default=30, help="Number of frames before Fall onset to label as Pre-fall. Default: 30.")
    parser.add_argument("--fall-threshold", type=float, default=0.72, help="Risk threshold used to mark Fall onset.")
    parser.add_argument("--prefall-threshold", type=float, default=0.45, help="Risk threshold used to mark Pre-fall onset.")
    args = parser.parse_args()

    csv_paths = sorted(Path(args.input_dir).glob("*.csv"))
    if not csv_paths:
        raise RuntimeError(f"No feature CSV files found in directory: {args.input_dir}")
    ...
    print(f"Annotation intervals: {len(intervals)}")
    print("Next step: review the CSV and verify the Pre-fall/Fall boundary for every fall-* sequence.")
    ...
    if prefall_start > 0:
        intervals.append(
            DraftInterval(
                ...
                label="Normal",
                method="draft",
                notes="Early frames before Fall onset; drafted as Normal.",
            )
        )
    ...
    if prefall_start < fall_start:
        intervals.append(
            DraftInterval(
                ...
                label="Pre-fall",
                method="draft",
                notes=f"The {prefall_frames} frames before Fall onset; drafted as Pre-fall.",
            )
        )
...
def guess_fall_start(
    ...
    for row in rows:
        if row.get("instant_state") == "Fall" and safe_float(row.get("has_pose")) > 0.0:
            frame = row_frame(row)
            return FallStartGuess(frame, "instant_state", "instant_state first reaches Fall.")
    ...
    for row in rows:
        if safe_float(row.get("smoothed_risk_score")) >= fall_threshold and safe_float(row.get("has_pose")) > 0.0:
            ...
            return FallStartGuess(frame, "smoothed_risk", f"smoothed_risk_score first reaches >= {fall_threshold:.2f}; current value: {score:.3f}.")

    for row in rows:
        if safe_float(row.get("risk_score")) >= fall_threshold and safe_float(row.get("has_pose")) > 0.0:
            frame = row_frame(row)
            score = safe_float(row.get("risk_score"))
            return FallStartGuess(frame, "risk_score", f"risk_score first reaches >= {fall_threshold:.2f}; current value: {score:.3f}.")
    ...
    if score < prefall_threshold:
        return FallStartGuess(frame, "max_risk_low_confidence", f"No clear Fall signal; using peak smoothed_risk_score={score:.3f}. Manual review is required.")
    return FallStartGuess(frame, "max_risk", f"Using peak smoothed_risk_score={score:.3f}.")
