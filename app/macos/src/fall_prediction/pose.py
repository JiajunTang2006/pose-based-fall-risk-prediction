...
# 默认模型文件路径：应用根目录下的 models/
# 仅在使用新版 MediaPipe Tasks API 时需要
_APP_ROOT = Path(__file__).resolve().parents[2]  # fall_prediction → src → macos/
DEFAULT_MODEL_PATH = _APP_ROOT / "models" / "pose_landmarker_full.task"
DEFAULT_YOLO_MODEL_PATH = _APP_ROOT / "models" / "yolo26n-pose.pt"
_YOLO_MODEL_LOCK = threading.RLock()
_YOLO_MODEL_CACHE: dict[Path, object] = {}
_YOLO_WARMED_MODELS: set[Path] = set()
...
def preload_yolo_model(model_path: str | Path | None = None, *, warmup: bool = True):
    """Load the YOLO model early so monitoring can start quickly."""
    model = Path(model_path) if model_path else DEFAULT_YOLO_MODEL_PATH
    return load_yolo_model(model, warmup=warmup)


def load_yolo_model(model_path: str | Path, *, warmup: bool = False):
    ...
    cache_key = model.resolve()

    with _YOLO_MODEL_LOCK:
        loaded = _YOLO_MODEL_CACHE.get(cache_key)
        if loaded is None:
            from .runtime import prepare_runtime_cache
            ...
            loaded = YOLO(str(cache_key))
            _YOLO_MODEL_CACHE[cache_key] = loaded

        if warmup and cache_key not in _YOLO_WARMED_MODELS:
            _warm_yolo_model(loaded)
            _YOLO_WARMED_MODELS.add(cache_key)
        return loaded


def _warm_yolo_model(model) -> None:
    import numpy as np
...
class YOLOPoseEstimator:
    ...
    def __init__(
        ...
        self._model = load_yolo_model(model)
